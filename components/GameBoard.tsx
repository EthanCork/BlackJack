...
export default function GameBoard() {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b ${theme.bgGradient} flex flex-col`}>
      ...
      {/* Main Game Area */}
      <main className="flex-1 flex flex-col items-center justify-between py-8 px-4 gap-8">
        {/* Dealer Area */}
        ...
        <div className="w-full max-w-5xl flex flex-col items-center gap-6">
          ...
          <div className={`w-full rounded-3xl bg-gradient-to-br ${theme.feltGradient} border-4 ${theme.borderColor} p-8 shadow-2xl ${theme.glowColor}`}>
            <div className="flex flex-col items-center gap-4">
              {/* Current Bet Display (during play) */}
              {currentBet > 0 && phase !== 'betting' && !isSplitActive && (
                <div className={`px-8 py-4 bg-black/40 rounded-full border-2 ${theme.borderColor}`}>
                  <div className="text-center">
                    <span className={`text-sm ${theme.accentColor} uppercase block`}>Bet</span>
